...
def train_cnn(optimizer, model, train_generator, valid_generator, criterion, args, upsamplers):
    ...
    for epoch in range(args.epoch + 1):
        # train_generator.sampler.set_epoch(epoch)
        model.train()
        for upsampler in upsamplers:
            upsampler.train()
        for iteration, (image, gt, scale) in enumerate(train_generator):
            loss = 0.
            for idx_scale in [0, 1, 2, 3]:
                ...
                if args.mixup and image_scale.shape[0] > 1:
                    image_scale, gt_scale = mixuper(image_scale, gt_scale)
