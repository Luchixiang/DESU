...
import numpy as np
import random
from moe_dataset import build_moe_dataset
...
def train_moe(args):
    ...
    for subdir in os.listdir(args.data):
        for tissue in os.listdir(os.path.join(args.data, subdir)):
            data_loader = build_moe_dataset(args, os.path.join(args.data, subdir, tissue))
            ...
            for epoch in range(num_epoch + 1):
                ...
                for idx, (img, target) in enumerate(data_loader):
                    ...
                    if (idx + 1) % 5 == 0:
                        ...
                        sys.stdout.flush()
                print(f'epoch{epoch} acc {correct / num_sample}')
            torch.save({
                'args': args,
                'epoch': num_epoch + 1,
                'state_dict': model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict()
            }, os.path.join(args.output, f'expert_{subdir}_{tissue}_last_acc{correct / num_sample}.pt'))
            print("Saving model ",
                  os.path.join(args.output, f'expert_{subdir}_{tissue}_last_acc{correct / num_sample}.pt'))
