...
def predict(im, net, upsampler, noiseModel, device, outScaling):
    '''
...
def tiledPredict(im, net, upsampler, ps, overlap, noiseModel, device, pad=False, outScaling=10.0):
    '''
    ...
    '''
    if pad:
        return tiledPredict_pad(im, net, upsampler, ps, overlap, noiseModel, device, outScaling)
    else:
        return tiledPredict_reflect(im, net, upsampler, ps, overlap, noiseModel, device, outScaling)


def tiledPredict_pad(im, net, upsampler, ps, overlap, noiseModel, device, outScaling=10.0):
    '''
    Tile the image to save GPU memory.
    Process it using our network.
    ...
    while (xmin < im.shape[1]):
        ...
        while (ymin < im.shape[0]):
            ...
            a, b = predict(inputPatch, net, upsampler, noiseModel, device, outScaling=outScaling)
...
def tiledPredict_reflect(im, net, upsampler, ps, overlap, noiseModel, device, outScaling=10.0):
    '''
    Tile the image to save GPU memory.
    Process it using our network.
    ...
    while (xmin < im.shape[1]):
        ...
        while (ymin < im.shape[0]):
            ...
            a, b = predict(im[ymin_:ymax, xmin_:xmax], net, upsampler, noiseModel, device, outScaling=outScaling)
