...
def trainingPred(my_train_data, net,upsampler, dataCounter, size, bs, numPix, device, augment=True, supervised=True):
    '''
    ...
    # print('before input to the network:', inputs_raw.shape)
    # Forward step
    outputs = net((inputs_raw - meanTorch) / stdTorch) # We found that this factor can speed up training
    outputs = upsampler(outputs) * 10.0
    # print('after the network:', outputs.shape)
...
def trainNetwork(net, upsampler, trainData, valData, noiseModel, postfix, device,
                 directory='.',
                 numOfEpochs=200, stepsPerEpoch=50,
                 batchSize=4, patchSize=128, learningRate=0.0001,
                 numMaskedPixels=128 * 128 / 32.0,
                 virtualBatchSize=20, valSize=20,
                 augment=True,
                 supervised=False, freeze=False
                 ):
    '''
    ...
    net.to(device)
    upsampler.to(device)
    optimizer = optim.Adam([{'params': net.parameters()}, {'params': upsampler.parameters()}], lr=learningRate)
    # optimizer = optim.SGD([{'params': net.parameters()}, {'params': upsampler.parameters()}], lr=learningRate)
    ...
    while stepCounter / stepsPerEpoch < numOfEpochs:  # loop over the dataset multiple times
        if freeze:
            if stepCounter / stepsPerEpoch <20:
        ...
        if stepCounter % stepsPerEpoch == stepsPerEpoch - 1:
            ...
            torch.save(upsampler, os.path.join(directory, "upsampler_last_" + postfix + ".net"))
            ...
            if len(valHist) == 0 or avgValLoss < np.min(np.array(valHist)):
                ...
                torch.save(upsampler, os.path.join(directory, "upsampler_best_" + postfix + ".net"))
