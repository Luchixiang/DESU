...
import pandas
from skimage.transform import resize
from unet import UNet, Unet_upsampler
...
class DataGenerator(data.Dataset):
    def __init__(self, root, num_exps=100, batch_size=4, dim=(128, 128), n_channels=1, shuffle=True, train=True,
                 validation=False, test=False, test_type=1, transform=None):
        'Initialization'
        self.root = root
        self.dim = dim
    ...
    def _gather_files(self):
        ...
        for fov in fovs:
            for file in os.listdir(os.path.join(self.root, f'FOV{fov}')):
                ...
                gt_list.append(
                    os.path.join(self.root, 'Target', f'W800_P200_6mW_Ax1_FOV_{str(fov).zfill(2)}_I_t1_SRRF.tif'))
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        for i, ID in enumerate(list_IDs_temp):
            x_img = imread(self.img_list[ID])
            y_label = imread(self.gt_list[ID])
            ...
            imx1 = np.array(
                [x_img[x:x + self.dim[0], y:y + self.dim[1]] for x in range(0, x_img.shape[0], self.dim[0]) for y in
                 range(0, x_img.shape[1], self.dim[1])])
            imx1 = np.array(imx1)
            # print('>>>> ',imx1.shape)
            lbx1 = np.array(
                [y_label[x:x + self.dim[0], y:y + self.dim[1]] for x in range(0, y_label.shape[0], self.dim[0]) for y in
                 range(0, y_label.shape[1], self.dim[1])])
            # print('>>>>>>>>>>>>>>> ',X.shape)
...
im_height = 128  # M
im_width = 128  # N
num_exps_train = 700  # total train examples = 57000 we are using all FOV 9 to 16 here which is 8*50 = 400 images
num_exps_validation = 40  # total test examples = 3000
num_exps_test = 50  # test dataset with raw data #1 FOV 8 image

params_train = {'dim': (im_height, im_width), 'num_exps': num_exps_train, 'batch_size': batch_size, 'n_channels': 1,
                'shuffle': True, 'train': True, 'validation': False, 'test': False, 'test_type': test_type,
                'transform': ToTensor}
...
params_test = {'dim': (im_height, im_width), 'num_exps': num_exps_test, 'batch_size': batch_size_test, 'n_channels': 1,
               'shuffle': False, 'train': False, 'validation': False, 'test': True, 'test_type': test_type,
               'transform': ToTensor}
...
transformed_dataset_train = DataGenerator(root='/userhome/34/cxlu/5/Dataset_SR/',**params_train)  # convert to tensor
dataloader_train = DataLoader(transformed_dataset_train, batch_size=1, shuffle=True,
                              num_workers=4)  # conver to data loader

# train_loader = training_generator
# print('len of train loader',len(dataloader_train))
# print('shape of train loader',dataloader_train)
# test_loader = test_generator
transformed_dataset_test = DataGenerator(root='/userhome/34/cxlu/5/Dataset_SR/', **params_test)  # convert to tensor
dataloader_test = DataLoader(transformed_dataset_test, batch_size=1, shuffle=False,
                             num_workers=4)  # conver to data loader
...
logger['rmse_train'] = []
logger['rmse_test'] = []
n_train_pixels = batch_size * im_height * im_width * 4 * len(dataloader_train) # slices
n_test_pixels = batch_size_test * im_height * im_width * 4 *len(dataloader_test) # here batch is only 1 sample
...
model = Unet_upsampler(res=False, weight='/userhome/34/cxlu/desu/best.pt').to(device)
#model =
...
def train(epochs):
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_idx, (noisy_input, clean_target) in enumerate(dataloader_train):
            ...
            loss = F.mse_loss(denoised_full, clean_full,
                              size_average=False)  # this is the error bwtween images of 256x256 MSE loss
            with torch.no_grad():
                psnr += cal_psnr(denoised_full, clean_full.detach()).sum().item()
            loss.backward()
        ...
        psnr = psnr / (len(dataloader_train) * batch_size * 4)
        scheduler.step(rmse)

        logger['rmse_train'].append(rmse)
        print("Epoch {} training RMSE: {:.6f}, PSNR:{:.6f}".format(epoch, rmse, psnr))
        ...
        with torch.no_grad():
            ...
            for batch_idx, (noisy, clean) in enumerate(dataloader_test):
                ...
                test_psnr += cal_psnr(denoised, clean).sum().item()
                ...
                if epoch % 50 == 0:
                    ...
                    io.savemat('predict_pressure_%d.mat' % epoch,
                               dict([('predict_pressure1_test', np.array(par_babu))]))

            rmse_test = np.sqrt(mse_test / n_test_pixels)
            test_psnr = test_psnr / (len(dataloader_test) * batch_size_test * 4)
            if test_psnr > best_psnr:
                best_psnr = test_psnr
            logger['rmse_test'].append(rmse_test)
            print("Epoch {}: test RMSE: {:.6f}, PSNR:{:.6f}".format(epoch, rmse_test, test_psnr))
