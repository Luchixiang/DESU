...
class Parser(argparse.ArgumentParser):
    def __init__(self):
        ...
        self.add_argument('--exp-name', type=str, default='dncnn_full_warm_loaded', help='experiment name')
        self.add_argument('--exp-dir', type=str, default="./experiment3", help='directory to save experiments')
        ...
        self.add_argument('--warm', action='store_true', default=False)
...
four_crop_test = transforms.Compose([
    transforms.FiveCrop(args.imsize),
    transforms.Lambda(lambda crops: torch.stack([
        fluore_to_tensor(crop) for crop in crops[:4]])),
    transforms.Lambda(lambda x: x.float().div(255).sub(0.5))
...
test_losses = [[]for _ in range(len(noise_levels))]
...
try:
        ...
        with torch.no_grad():
            ...
            if epoch % 10 == 0 or epoch == args.epochs:
                for noise_level in noise_levels:
                    test_loader = load_denoising_test_mix(args.data_root,
                                                          batch_size=2, noise_levels=[noise_level],
                                                          transform=four_crop_test, target_transform=four_crop_test,
                    ...
                    test_losses[noise_levels.index(noise_level)].append(psnr)
                    result = {'psnr': psnr,
                              'ssim': ssim,
                              'time': time_taken}
    ...
    with open(args.ckpt_dir + '/test_psnr.pkl', 'wb') as f:
