...
def batch_SSIM(img, imclean, data_range):
    ...
    for i in range(Img.shape[0]):
        if Img.shape[1] > 1: #if color image, swap color channels to last dimension and use multi-channel ssim
            ...
            SSIM += metrics.structural_similarity(np.moveaxis(Iclean[i,:,:,:],0,-1), np.moveaxis(Img[i,:,:,:],0,-1), data_range=data_range, multichannel=True)
        else:
            # print(Iclean[i].shape)
            SSIM += metrics.structural_similarity(Iclean[i,0,:,:], Img[i,0,:,:], data_range=data_range)
